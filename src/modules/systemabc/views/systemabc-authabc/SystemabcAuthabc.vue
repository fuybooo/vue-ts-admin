...
<script lang="ts">
  ...
  @Component({})
  export default class SystemabcAuthabc extends Vue {
    ...
    // 跳转到创建页面
    public handleClick () {
      this.$router.push({name: 'systemabc-authabc-detail', params: {pattern: 'create'}})
    }
    // 执行删除
    public del (row: any) {
    ...
    public toEdit (row: any) {
      this.$router.push({name: 'systemabc-authabc-detail', params: {pattern: 'edit', id: row.id}})
    }
  }
</script>
