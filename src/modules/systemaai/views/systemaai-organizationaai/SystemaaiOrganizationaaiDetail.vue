...
<script lang="ts">
  ...
  @Component({})
  export default class SystemaaiOrganizationaaiDetail extends Vue {
      ...
      (this.$refs.form as any).$refs.form.validate((valid: boolean) => {
        if (valid) {
          this.$req(this.$urls.user[this.formPattern === 'create' ? 'create' : 'update'], {...this.form, ...(this.id ? {id: this.id} : {})}).then((res: HttpRes) => {
            if (res.head.errCode === 0) {
              this.id = res.data.id
              this.$router.push({name: 'systemaai-organizationaai-detail', params: {pattern: 'view', id: res.data.id}})
