...
<script lang="ts">
  ...
  @Component({})
  export default class SystemaauOrganizationaau extends Vue {
    ...
    // 跳转到创建页面
    public handleClick () {
      this.$router.push({name: 'systemaau-organizationaau-detail', params: {pattern: 'create'}})
    }
    // 执行删除
    public del (row: any) {
    ...
    public toEdit (row: any) {
      this.$router.push({name: 'systemaau-organizationaau-detail', params: {pattern: 'edit', id: row.id}})
    }
  }
</script>
