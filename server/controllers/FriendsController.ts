...
import FriendsInstance from '../models/FriendsInstance'
import {Ctx, SchemaMap} from '../types'

export default class FriendsController extends BaseController {
  public Model: FriendsModel
  public schemaMap!: SchemaMap
  constructor (ctx: Ctx) {
    super(ctx)
    this.Model = instance.getInstance(FriendsModel)
    this.createSchema()
  }
  private async list (ctx: Ctx) {
    const results: FriendsInstance[] = await this.Model.list(
      {
        ...ctx.params,
        findParams: {
          $or: [
  ...
  private async create (ctx: Ctx) {
    const result: FriendsInstance = await this.Model.findBy({code: ctx.params.code})
    if (result) {
      return (ctx.body = resReturn(null, 405, '唯一编码重复'))
    }
    try {
      const friends: FriendsInstance = await this.Model.create({
        name: ctx.params.name,
        code: ctx.params.code,
        parentId: ctx.params.parentId,
      })
      ctx.body = resReturn(friends, 0, '创建成功')
    } catch (e) {
      ctx.body = resReturn(null, 500, e.message)
    }
  }
  private async update (ctx: Ctx) {
    const result: FriendsInstance = await this.Model.get(ctx.params.id)
    if (!result) {
  ...
  private async get (ctx: Ctx) {
    const friends: FriendsInstance = await this.Model.get(ctx.params.id)
    ctx.body = resReturn(friends)
  }
