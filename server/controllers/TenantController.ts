...
import TenantInstance from '../models/TenantInstance'
import {Ctx, SchemaMap} from '../types'

export default class TenantController extends BaseController {
  public Model: TenantModel
  public schemaMap!: SchemaMap
  constructor (ctx: Ctx) {
    super(ctx)
    this.Model = instance.getInstance(TenantModel)
    this.createSchema()
  }
  private async list (ctx: Ctx) {
    const results: TenantInstance[] = await this.Model.list(
      {
        ...ctx.params,
        findParams: {
          $or: [
  ...
  private async create (ctx: Ctx) {
    const result: TenantInstance = await this.Model.findBy({code: ctx.params.code})
    if (result) {
  ...
  private async update (ctx: Ctx) {
    const result: TenantInstance = await this.Model.get(ctx.params.id)
    if (!result) {
