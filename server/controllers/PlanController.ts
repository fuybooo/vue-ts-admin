...
export default class PlanController extends BaseController {
  ...
  constructor (ctx: Ctx) {
    super(ctx)
    this.Model = instance.getInstance(PlanModel)
    this.createSchema()
  }
  private async list (ctx: Ctx) {
    const results: PlanInstance[] = await this.Model.list(
      {
        ...ctx.params,
        findParams: {
          $or: [
  ...
  private async create (ctx: Ctx) {
    const result: PlanInstance = await this.Model.findBy({code: ctx.params.code})
    if (result) {
      return (ctx.body = resReturn(null, 405, '唯一编码重复'))
    }
    try {
      const plan: PlanInstance = await this.Model.create({
        name: ctx.params.name,
        code: ctx.params.code,
        parentId: ctx.params.parentId,
      })
      ctx.body = resReturn(plan, 0, '创建成功')
    } catch (e) {
      ctx.body = resReturn(null, 500, e.message)
    }
  }
  private async update (ctx: Ctx) {
    const result: PlanInstance = await this.Model.get(ctx.params.id)
    if (!result) {
  ...
  private async get (ctx: Ctx) {
    const plan: PlanInstance = await this.Model.get(ctx.params.id)
    ctx.body = resReturn(plan)
  }
