...
class FriendsController extends BaseController_1.default {
    constructor(ctx) {
        super(ctx);
        this.Model = instance_1.default.getInstance(FriendsModel_1.default);
        this.createSchema();
    }
    async list(ctx) {
    ...
    async create(ctx) {
        ...
        try {
            const friends = await this.Model.create({
                name: ctx.params.name,
                code: ctx.params.code,
                parentId: ctx.params.parentId,
            });
            ctx.body = intercept_1.resReturn(friends, 0, '创建成功');
        }
        catch (e) {
    ...
    async get(ctx) {
        const friends = await this.Model.get(ctx.params.id);
        ctx.body = intercept_1.resReturn(friends);
    }
    createSchema() {
