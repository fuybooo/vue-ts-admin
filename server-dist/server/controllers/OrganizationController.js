...
class OrganizationController extends BaseController_1.default {
    constructor(ctx) {
        super(ctx);
        this.Model = instance_1.default.getInstance(OrganizationModel_1.default);
        this.createSchema();
    }
    async list(ctx) {
    ...
    async create(ctx) {
        ...
        try {
            const organization = await this.Model.create({
                name: ctx.params.name,
                code: ctx.params.code,
                parentId: ctx.params.parentId,
            });
            ctx.body = intercept_1.resReturn(organization, 0, '创建成功');
        }
        catch (e) {
    ...
    async get(ctx) {
        const organization = await this.Model.get(ctx.params.id);
        ctx.body = intercept_1.resReturn(organization);
    }
    async upload(ctx) {
        if (ctx.request.files) {
            const file = ctx.request.files.file;
            FileService_1.default.readFile(file);
        }
        ctx.body = intercept_1.resReturn(null, 0, '测试结果');
    }
    createSchema() {
