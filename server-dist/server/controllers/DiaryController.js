...
const DiaryModel_1 = require("../models/DiaryModel");
const intercept_1 = require("../utils/intercept");
class DiaryController extends BaseController_1.default {
    constructor(ctx) {
        super(ctx);
        this.Model = instance_1.default.getInstance(DiaryModel_1.default);
        this.createSchema();
    }
    async list(ctx) {
    ...
    async create(ctx) {
        ...
        try {
            const diary = await this.Model.create({
                name: ctx.params.name,
                code: ctx.params.code,
                parentId: ctx.params.parentId,
            });
            ctx.body = intercept_1.resReturn(diary, 0, '创建成功');
        }
        catch (e) {
    ...
    async get(ctx) {
        const diary = await this.Model.get(ctx.params.id);
        ctx.body = intercept_1.resReturn(diary);
    }
    createSchema() {
