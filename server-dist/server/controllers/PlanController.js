...
class PlanController extends BaseController_1.default {
    constructor(ctx) {
        super(ctx);
        this.Model = instance_1.default.getInstance(PlanModel_1.default);
        this.createSchema();
    }
    async list(ctx) {
    ...
    async create(ctx) {
        ...
        try {
            ...
            ctx.body = intercept_1.resReturn(plan, 0, '创建成功');
        }
        catch (e) {
    ...
    async get(ctx) {
        const plan = await this.Model.get(ctx.params.id);
        ctx.body = intercept_1.resReturn(plan);
    }
    createSchema() {
